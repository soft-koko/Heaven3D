...
template<> 
Heaven3D::H3DRenderer* Heaven3D::H3DSingleton<Heaven3D::H3DRenderer>::ms_pkSingleton = 0;
...
namespace Heaven3D
{
	H3DRenderer::H3DRenderer()
	{
		swap_chain_ = 0;
		d3d_device_ = 0;
		d3d_device_context_ = 0;
		back_buffer_view_ = 0;
		depth_stencil_state_ = 0;
		rasterizer_state_ = 0;
		depth_disable_stencil_state_ = 0;
...
// ----------------------
	...
	void H3DRenderer::Shutdown()
	{
		...
		depth_stencil_zone_.Cleanup();

		H3D_SAFE_RELEASE(alpha_enable_blending_state_);
		H3D_SAFE_RELEASE(alpha_disable_blending_state_);
		H3D_SAFE_RELEASE(depth_disable_stencil_state_);
		H3D_SAFE_RELEASE(depth_stencil_state_);
		H3D_SAFE_RELEASE(back_buffer_view_);
		H3D_SAFE_RELEASE(d3d_device_context_);
		H3D_SAFE_RELEASE(d3d_device_);
		H3D_SAFE_RELEASE(swap_chain_);
	}

	void H3DRenderer::BeginScene(float red, float green, float blue, float alpha)
	{
		float color[4];
		color[0] = red;
		color[1] = green;
		...
		// 清空backbuffer和深度模板缓冲区
		d3d_device_context_->ClearRenderTargetView(back_buffer_view_, color);
		//d3d_device_context_->ClearDepthStencilView(depth_stencil_view_, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);
		depth_stencil_zone_.ClearView();
	}

	void H3DRenderer::TurnOnAlphaBlending()
	{
		float blendFactor[4];
		blendFactor[0] = 0.0f;
		blendFactor[1] = 0.0f;
		blendFactor[2] = 0.0f;
		blendFactor[3] = 0.0f;
		d3d_device_context_->OMSetBlendState(alpha_enable_blending_state_, blendFactor, 0xffffffff);
	}

	void H3DRenderer::TurnOffAlphaBlending()
	{
		float blendFactor[4];
		blendFactor[0] = 0.0f;
		blendFactor[1] = 0.0f;
		blendFactor[2] = 0.0f;
		blendFactor[3] = 0.0f;
		d3d_device_context_->OMSetBlendState(alpha_disable_blending_state_, blendFactor, 0xffffffff);
	}
}
